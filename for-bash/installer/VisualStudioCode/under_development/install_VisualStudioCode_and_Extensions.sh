...
export  g_VisualStudioCodeVersion="1.40.0"
	#// https://code.visualstudio.com/updates/v1_40
	#// 1.34, 1.35 and 1.36 has problems in settings-sync plug-in.

export  g_UserOrAllUsers="User"
	#// "User" or ""(All users)

export  g_CodeSettingsSyncVersion="3.2.9"
	#// "User" or ""(All users)

export  g_Rollback="${True}"
	#// True or False
#//==================================================================
	...
	if [ ! -e "${code_cmd_path}" ];then
		if [ ! -e "${installer_file_name}" ]; then
			local  error_message=$(cat <<- __HERE_DOCUMENT__
				Not found '${installer_file_name}' or '${code_cmd_path}'
		...
		"./sub_command_from_cmd_exe.bat"  "${installer_file_name}"  slash SILENT
		echo  ""
		echo  "Please, close Visual Studio Code window."
		Pause_func

		"${code_cmd_path}"  --install-extension  "Shan.code-settings-sync@${g_CodeSettingsSyncVersion}"
	fi


	#// ...
	echo  ""
	ColorEcho_func  "List up extensions ...\n"  "Green"
	EchoNextCommand_func

	"${code_cmd_path}"  --list-extensions  --show-versions
	#// "${code_cmd_path}"  --install-extension
	#// "${code_cmd_path}"  --uninstall-extension


	#// ...
	echo  ""
	ColorEcho_func  "Upload extension setting ...\n"  "Green"
	echo  "Push [Alt]+[Shift]+[U]"
	"${code_cmd_path}"
	Pause_func


	#// Uninstall
	if [ "${g_Rollback}" == "${True}" ]; then

		EchoNextCommand_func

		"./sub_command_from_cmd_exe.bat"  "${uninstaller_file_path}"  slash SILENT
		EchoNextCommand_func
...
#********************************************************************
export  g_ThisScriptPath="$0"
GetFullPath_func  "${g_ThisScriptPath}"
